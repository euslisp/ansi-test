...
To run the tests (assuming you have installed ~euslisp~, perhaps with the ~irt~ extension),
issue either ~roseus run-tests.l~, ~irteusgl run-tests.l~, ~eus run-tests.l~ or ~./run-tests.l~
(assuming the file has execution permissions).
